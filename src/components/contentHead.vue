...
<template>
  <div class="content-head-wrapper">
    ...
    <div class="content-head-network">
      <a target="_blank" href="javascript:void(0)">{{this.$t("head.helpText")}}</a>
      ...
      <el-dropdown trigger="click" @command="changeGroup" placement="bottom">
        <span class="cursor-pointer font-color-fff" @click="groupVisible = !groupVisible">
          ...
          <i :class="[groupVisible?'el-icon-arrow-up':'el-icon-arrow-down']"></i>
        </span>
        <el-dropdown-menu slot="dropdown">
          <li
            class="cursor-pointer font-color-2956a3 text-center"
            @click="goGroupMgmt"
            v-if="root==='admin'"
          >{{this.$t('title.groupManagement')}}</li>
      ...
      <el-popover placement="bottom" width="0" min-width="50px" trigger="click">
        <div class="sign-out-wrapper">
          <span class="change-password" @click="changePassword">{{this.$t("head.changePassword")}}</span>
          <br />
